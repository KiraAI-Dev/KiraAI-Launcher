...
const projectSettingsHost = ref('0.0.0.0')
...
async function installEnvironmentTool(tool: EnvironmentTool) {
  if (tool.installed || environmentChecking.value || isEnvironmentInstalling(tool)) return
...
function openProjectSettings(project: ManagedProject) {
  ...
  projectSettingsHost.value = project.type === 'local' ? project.host ?? '0.0.0.0' : '0.0.0.0'
...
async function saveProjectSettings() {
  ...
  try {
    const updatedProject = project.type === 'local'
      ? await requireLauncherBridge().projects.update({ id: project.id, name: projectSettingsName.value, host: projectSettingsHost.value, port: projectSettingsPort.value, launchArgs: parseProjectSettingsLaunchArgs(), environmentVariables: parseProjectSettingsEnvironmentVariables() })
    ...
    <n-message-provider>
      ...
      <n-layout native-scrollbar class="right-layout">
        <n-layout-content :native-scrollbar="false" class="right-content" content-style="padding: 32px 38px;">
          ...
          <template v-else-if="activeView === 'logs'"><n-space vertical :size="24" class="logs-page"><div class="page-heading"><div><h1>{{ t.logs }}</h1><p>{{ t.logsSub }}</p></div><n-button :loading="logsLoading" @click="refreshLauncherLog"><template #icon><n-icon :component="RefreshOutline" /></template>{{ t.refreshLogs }}</n-button></div><n-alert v-if="logsError" type="error" :title="t.operationFailed" closable @close="logsError = ''">{{ logsError }}</n-alert><div v-if="!launcherLog.content" class="log-empty"><n-empty :description="t.noLogs" size="small" /></div><pre v-else class="log-output" tabindex="0" :aria-label="t.logs">{{ launcherLog.content }}</pre></n-space></template>
