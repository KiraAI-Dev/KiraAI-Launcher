...
- 检查 Python、uv 和 Node.js 的安装状态，并可安装缺失工具的受支持版本。
...
管理本地项目时，请先安装受支持版本的 Python，并将其加入 `PATH`。你可以在 **环境** 页面查看 Python、uv 和 Node.js 是否可用，并通过系统包管理器安装缺失工具。安装前可选择版本；完成后请重启启动器，让新的命令行路径生效。
